...
# This script computes pairwise similarity scores between real and synthetic image embeddings.
# It dynamically selects the feature extractor, computes the dense score matrix,
# and exports the latent vectors along with the indexing metadata for downstream evaluation.
# Authors: Antonio Scardace and Lemuel Puglisi

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--augment',            action='store_true', help='Enable data augmentation during feature extraction.')
    parser.add_argument('--use_gpu',            action='store_true', help='Enable GPU acceleration for model inference.')
    parser.add_argument('--metric_name',        type=str, required=True, choices=['dar', 'ums_sscd', 'deepssim'])
    ...
    parser.add_argument('--embeddings_dir',     type=str, required=True, help='Output directory to save the extracted embeddings.')
    parser.add_argument('--matrices_dir',       type=str, required=True, help='Output directory to save the computed score matrix.')
    parser.add_argument('--indices_dir',        type=str, required=True, help='Output directory to save the real and synthetic index metadata.')
    parser.add_argument('--batch_size',         type=int, default=32,    help='Number of samples per batch during inference.')
    ...
    # Instantiates the designated metric factory to construct the feature extractor and the scorer.
    # Allocates the model onto the selected hardware device.
    ...
    # Gathers image paths and their corresponding unique identifiers (UIDs) from the dataset directory.
    # Processes the images in batches, computing and saving individual embeddings as compressed files.
    ...
    # Partitions the saved embeddings into training and synthetic subsets based on their file suffix.
    # Computes the cross-similarity score matrix and exports it alongside the associated index metadata.
    # Each matrix entry (i, j) stores the score between the i-th training and j-th synthetic embedding.
    ...
    np.savez(os.path.join(args.matrices_dir, args.metric_name + is_augmented + '.npz'), data=score_matrix.astype(np.float32))
